...
import pygame
# Es wird angenommen, dass config_retro.py existiert und die UI-Farben enthält.
import config_retro as cfg

class SliderManager:
    def __init__(self, screen, height): # height ist Bildschirmhöhe
        self.screen = screen
        ...
        self.avoidance_strength = cfg.DEFAULT_AVOIDANCE

        base_y = height - self.slider_y_offset
        
        self.avoidance_slider_rect = pygame.Rect(
            self.slider_padding,
            base_y - 1 * (self.slider_height + self.slider_padding),
            self.slider_width,
            self.slider_height,
        )
        self.speed_slider_rect = pygame.Rect(
            self.slider_padding,
            base_y - 2 * (self.slider_height + self.slider_padding),
            self.slider_width,
            self.slider_height,
        )
        self.separation_slider_rect = pygame.Rect(
            self.slider_padding,
            base_y - 3 * (self.slider_height + self.slider_padding),
            self.slider_width,
            self.slider_height,
        )
        self.alignment_slider_rect = pygame.Rect(
            self.slider_padding,
            base_y - 4 * (self.slider_height + self.slider_padding),
            self.slider_width,
            self.slider_height,
        )
        self.cohesion_slider_rect = pygame.Rect(
            self.slider_padding,
            base_y - 5 * (self.slider_height + self.slider_padding),
            self.slider_width,
            self.slider_height,
        )
        ...
        # self.close_button_rect wurde entfernt
        
        self.mouse_over_apply = False
        # self.mouse_over_close wurde entfernt
    ...
    def draw_sliders_and_buttons(self): # Name kann beibehalten oder zu draw_ui_elements geändert werden
        ...
        # Das Zeichnen des Close-Buttons wurde entfernt
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1: 
                ...
                if not slider_clicked_this_event: 
                    if self.mouse_over_apply:
                        action_to_return = "apply"
                    # Die Prüfung für mouse_over_close wurde entfernt
            ...
            mouse_x = mouse_pos[0]
            slider_map = {
                "cohesion": ("cohesion_strength", self.cohesion_slider_rect),
    ...
    def apply_values_to_birds(self, birds_group_or_list):
        for bird in birds_group_or_list:
            ...
            bird.avoidance_strength = self.avoidance_strength