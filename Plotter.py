...
class GamePlotter:
    ...
    def open_graph_window(self, time_steps, data_series_map):
        with self.graph_lock:
            if self.is_graph_showing and self.fig: # If already showing, try to activate
                try:
                    if hasattr(self.fig.canvas.manager, 'window') and self.fig.canvas.manager.window:
        ...
        with self.graph_lock: # Protect Matplotlib objects
            if self.fig:
                try:
                    ...
                    pass # Ignore errors if already closed or in a bad state
            self.fig = None
            self.ax = None
            self.plot_lines = {}
    ...
    def is_window_alive(self):
        with self.graph_lock: # Ensure thread-safe check
            return bool(self.fig and self.is_graph_showing and plt.fignum_exists(self.fig.number))
    ...
    def queue_new_plot_data(self, time_steps, data_series_map):
        if self.is_graph_showing and (self.plot_thread and self.plot_thread.is_alive()):
            try:
                ...
                data_to_queue = (list(time_steps), {k: list(v) for k, v in data_series_map.items()})
    ...
    def _plotting_worker(self):
        ...
        while not self.stop_event.is_set():
            try:
                plot_job = self.plot_data_queue.get(timeout=0.1) # Timeout to check stop_event
                if plot_job is None: # Sentinel to stop the thread
                    break
                
                time_steps, data_series_map = plot_job
                self.redraw_all_graph_data(time_steps, data_series_map)
                
                # Allow Matplotlib's event loop to process GUI updates from this thread
                with self.graph_lock:
                    if self.fig and self.is_graph_showing:
                        try:
                            pass # Rely on self.fig.canvas.draw_idle() in redraw_all_graph_data
                        except Exception:
                            # Ignore if figure is closed or in an unstable state
                            pass
            except queue.Empty:
                continue # No data, loop again and check stop_event
            except Exception:
                # Log or handle other unexpected errors in the worker thread
                break # Exit thread on other errors for safety
